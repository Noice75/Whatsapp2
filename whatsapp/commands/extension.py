...
class setup_extension:
    ...
    def build(self):
        ...
        global cc
        cc = {}
        for file_name,i in cmdD.items():
            for func_name, func_ref in i[0].items():
                if(func_name in cc):
                    try:
                        raise Exception(f"Duplicate alias '{func_name}' in commands {cc[func_name].__name__} and {cmdD[file_name][0][func_name].__name__}")
                    except AttributeError:
                        raise Exception(f"Duplicate alias '{func_name}' in commands {cc[func_name][0].__name__} and {cmdD[file_name][0][func_name].__name__}")
                cc[func_name] = func_ref
            
            for class_name, class_func in i[1].items():
                for method_name, method_ref in class_func[0].items():
                    if(method_name in cc):
                        try:
                            raise Exception(f"Duplicate alias '{method_name}' in commands {cc[method_name][0].__name__} and {method_ref.__name__}")
                        except TypeError:
                            raise Exception(f"Duplicate alias '{method_name}' in commands {cc[method_name].__name__} and {method_ref.__name__}")
                    try:
                        if(len(class_func) > 1):
                            cc[method_name] = (method_ref,class_func[1])
                        else:
                            cc[method_name] = (method_ref,self.classesref[class_name])
                            cmdD[file_name][1][class_name].append(self.classesref[class_name])
                    except KeyError:
