...
def onReady():
    ...
    for filename in os.listdir("./commands"):
        if filename.endswith(".py"):
            commands.load_extension(f"commands/{filename[:-3]}")
    whatsapp.openChat("1234567890")

@commands.command(aliases=["Unload"])
def unload(ctx):
    file_name = ctx.body.split()[-1]
    if(commands.unload_extension(file_name)):
        whatsapp.send(f"Unloaded {file_name}")

@commands.command(aliases=["Load"])
def load(ctx):
    file_name = ctx.body.split()[-1]
    if(commands.load_extension(f"commands/{file_name}")):
...
def loadAll(ctx):
    for file_name in os.listdir("./commands"):
        if file_name.endswith(".py"):
            commands.load_extension(f"commands/{file_name[:-3]}")
...
def unloadAll(ctx):
    for file_name in os.listdir("./commands"):
        if file_name.endswith(".py"):
            commands.unload_extension(f"{file_name[:-3]}")
